...
public class IntermittentEmiters : MonoBehaviour
{
    ...
    void PlaySound() {
        ...
        intermittent.Play();
        StartCoroutine("WaitForIt", (audioFiles[index].length));
    }


    //This coroutine is going to wait by a random amout of time between min/max specified by the player.
    IEnumerator WaitForIt(float duration){



        yield return new WaitForSeconds(Random.Range(minTime, maxTime) + duration);
        PlaySound();
